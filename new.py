...
arduinoData = serial.Serial('COM10', 9600) #Change this based on your serial port
...
class VideoStream:
    ...
    def __init__(self,resolution=(640,640),framerate=30):
        ...
        self.stream = cap = cv2.VideoCapture('http://192.168.137.20:81/stream')
...
while True:
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            send_coordinates_to_arduino(xmin, ymin, xmax, ymax)

            # Draw label
            object_name = labels[int(classes[i])] # Look up object name from "labels" array using class index
            label = 'ID %d: %s: %d%%' % (detection_count, object_name, int(scores[i]*100)) # Example: 'ID 1: person: 72%'
